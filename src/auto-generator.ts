  ...
  constructor(tableData: TableData, dialect: DialectOptions, options: AutoOptions) {
    ...
    makeHeaderTemplate() {
        ...
        if (this.options.lang === 'ts') {
            ...
            header += "import { DataTypes, Optional } from 'sequelize';\n";
            header += `import {
    AllowNull,
    AutoIncrement,
    BelongsTo,
...
`;
      ...
      const tableName = makeTableName(this.options.caseModel, tableNameOrig, this.options.singularize, this.options.lang);
        ...
        const needed = _.keys(associations.needed).sort();
          ...
          str += 'import { ';
                ...
                str += "\ninterface #TABLE#Attributes {\n";
                str += this.addTypeScriptFields(table, true) + "}\n\n";

                const primaryKeys = this.getTypeScriptPrimaryKeys(table);

                if (primaryKeys.length) {
                    str += `type #TABLE#Pk = ${primaryKeys.map((k) => `"${(0, types_1.recase)(this.options.caseProp, k)}"`).join(' | ')};\n`;
                    str += `type #TABLE#Id = #TABLE#[#TABLE#Pk];\n`;
                }

                const creationOptionalFields = this.getTypeScriptCreationOptionalFields(table);

                if (creationOptionalFields.length) {
                    str += `type #TABLE#OptionalAttributes = ${creationOptionalFields.map((k) => `"${(0, types_1.recase)(this.options.caseProp, k)}"`).join(' | ')};\n`;
                    str += "type #TABLE#CreationAttributes = Optional<#TABLE#Attributes, #TABLE#OptionalAttributes>;\n\n";
                }
                else {
                    str += "type #TABLE#CreationAttributes = #TABLE#Attributes;\n\n";
                }
                str += `@Table({ tableName: "${tableNameOrig}", timestamps: true })\nexport class #TABLE# extends Model<#TABLE#Attributes, #TABLE#CreationAttributes> implements #TABLE#Attributes {\n`;
  ...
  private addTable(table: string) {
    ...
    addFieldTSDecorators(table: string, field: string): string {
    ...
    if (_.isObject(foreignKey)) {
        ...
        fieldAttrs.forEach(attr => {
            ...
            if (isSerialKey && !wroteAutoIncrement) {
                str += `${this.space[1]}@AutoIncrement\n`;
                wroteAutoIncrement = true;
            }
            if (attr === "foreignKey") {
                if (foreignKey && foreignKey.isForeignKey) {
                    let ref_tablename = (0, types_1.makeTableName)(this.options.caseModel, fieldObj[attr].foreignSources.target_table, this.options.singularize, this.options.lang);
                    str += `${this.space[1]}@ForeignKey(() => ${ref_tablename})\n`;
                }
                else {
                    return true;
                }
                ...
                str += `${this.space[1]}@Default(${val_text})\n`;
            }
                ...
                if (attr === "comment") {
                    str += `${this.space[1]}@Comment(${val})\n`;
                } else if (attr === "type") {
                    // TS uses DataType rather then DataTypes.                    
                    val = val.replace('DataTypes', 'DataType');
                    colStr += `${this.space[1]}@Column(${val})\n`;
                } else {
        ...
        if (unique) {
            ...
            // Note: if we want to support options other then unique=true then we might want this line below.
            // str += space[3] + "unique: " + uniq + ",\n";
            str += `${this.space[1]}@Unique\n`;
        }
        if (field !== fieldName) {
            str += space[3] + "field: '" + field + "',\n";
        }

        let idx = this.indexes[table].find(x => x.fields.find(y => y.attribute == fieldName));
        if (idx && idx.name != "PRIMARY" && !idx.name.startsWith("fk_")) { 
            // Primary Key and Foreign Key indexes are handled with their own
            // decorators above.
            str += `${this.space[1]}@Index({\n`;
            if (idx.name) {
                str += `${this.space[2]}name: "${idx.name}",\n`;
            }
            if (idx.unique) {
                str += `${this.space[2]}unique: true,\n`;
            }
            if (idx.type) {
                if (['UNIQUE', 'FULLTEXT', 'SPATIAL'].includes(idx.type)) {
                    str += `${this.space[2]}type: "${idx.type}",\n`;
                }
                else {
                    str += `${this.space[2]}using: "${idx.type}",\n`;
                }
            }
            // @TODO: Handle field collate, length, and order.
            str += `${this.space[1]}})\n`;
        }
    ...
    // Create a string containing field attributes (type, defaultValue, etc.)
    addField(table: string, field: string): string {
        // ignore Sequelize standard fields
        const additional = this.options.additional;
        if (additional && (additional.timestamps !== false) && (this.isTimestampField(field) || this.isParanoidField(field))) {
            return '';
  ...
  private getSqType(fieldObj: Field, attr: string): string {
    ...
    const type: string = attrValue.toLowerCase();
      ...
      val = 'DataTypes.CHAR' + (!_.isNull(length) ? length : '');
            ...
            val = 'DataTypes.REAL';
    } else if (type.match(/longtext$/)) {
            val = 'DataTypes.TEXT("long")';
    ...
    const needed: Record<string, Set<String>> = {};
        ...
        this.relations.forEach(rel => {
          ...
          const pparent = _.upperFirst(rel.parentProp);
                    ...
                    str += `${sp}@BelongsTo(() => ${rel.parentModel})\n`;
      ...
      if (!this.options.skipFields || !this.options.skipFields.includes(field)){
        const name = this.quoteName(recase(this.options.caseProp, field));
                ...
                if (!isInterface) {
                    ...
                    str += this.addFieldTSDecorators(table, field);
                }
